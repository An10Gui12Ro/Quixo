from TemplateMAB import QuixoBotMAB
from PlayerMAB import QuixoBot
import copy
...
class QuixoReferee:
    ...
    #HACE TODOS LOS POSIBLES MOVIMIENTOS DE ESA CASILLA SEGUN SU FILA O COLUMNA, SI ALGUNO COINCIDE CON EL 'new_board' DEJA ESE MOVIEMIENTO EN EL 'self.board'
    # Y REGRESA TRUE, SI NO, INVIERTE ESE MOVIMIENTO Y PRUEBA CON OTROS DESDE LA MISMA CASILLA, SI NINGUNO COINCIDE REGRESA EL SIMBOLO DE 
    # ESA CASILLA AL ORIGINAL, Y SI NINGUN MOVIMIENTO COINCIDIÓ SIGNIFICA QUE ES UN MOVIMIENTO INVALIDO
    #DEPENDIENDO DEL MOVIMIENTO QUE SE HAGA (down, up, right, left) SERAN LOS PARAMETROS PARA 'row', 'col', 'end_row' Y 'end_col'
    def __compare_with_movements_top_row(self, new_board, symbol):
        for i in range(5):
            #HACE ESTO PARA SABER SI ES UNA CASILLA QUE PUEDO MOVER SEGUN MI SIMBOLO
    ...
    def play_game(self, limit_turns):
        #CREO QUE EN EL 'reset' NO ES NECESARIO RESETEAR EL 'board' PORQUE YA LO RESETEA AQUI
        #YA SÉ DONDE MODIFICA EL 'self.board'
        self.board = [[0] * 5 for _ in range(5)]

        for i in range(2 * limit_turns):
            if i % 2 == 0:
                print('------------', (i + 2) // 2, ' TURN ------------') #AQUI SOLO IMPRIME CUANDO JUEGA EL 'self.player1'
    ...
    def play_match(self, limit_games, limit_turns):
        ...
        for i in range(limit_games):
            ...
            self.player1.who_win(winner) #AÑADÍ ESTO
