...
class Heuristics_H6:
    ...
    #Esto significa que dentro de sus mejores hay una posibilidad que haga un movimiento que me beneficie, entonces, de todas las posibilidades
    # que me convendrían (ya después de haber hecho todos los movimientos) escojeré la que más me beneficie. Esto no significa que él haga el movimiento
    # que me benefició, pero sí lo estoy guiando por el camino que yo quiero, además de que existe la posibilidad de que lo haga y más si sus mejores no se repiten
    #H6
    @staticmethod
    def heuristic_6(node, my_symbol, opponent):
        ...
        #Estoy 100% seguro de que la 'pq_h' nunca estaría vacía ya que tendrá todos los movimientos que se hicieron antes
        move = pq_h.get()
        x = move.heuristic_value[0]
        while move.heuristic_value[0] == x:
            ...
            #Esto sirve para que no se pierda ningún nodo ni tampoco se cicle al tratar de sacar un nodo de una PriorityQueue vacía
            if not pq_h.empty():
                move = pq_h.get()
            else:
                break
        ...
        # 3 1 2 3 , 4 4 4 4
        tuple = pq_2h.get() #Estoy 100% seguro de que la 'pq_2h' siempre tendrá algún nodo que sacar ya que al principio entra de cajón el primerito que saca la 'pq_h'
...
class World_H6:
    ...
    def bot(self, board, heuristic):
        ...
        #H6
        #Estoy 100% seguro de que la 'pq' nunca estaría vacía ya que tendrá todos los movimientos que se hicieron antes
        move = pq.get()
        if move.heuristic_value[0] != math.inf:
            ...
            my_move = pq_2.get() #Estoy 100% seguro de que la 'pq_2' siempre tendrá algún nodo que sacar ya que al principio entra de cajón el primerito que saca la 'pq'
        else:
            my_move = move
        #H6
